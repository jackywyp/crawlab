...
func SetEnv(cmd *exec.Cmd, envs []model.Env, task model.Task, spider model.Spider) *exec.Cmd {
	...
	cmd.Env = append(cmd.Env, "CRAWLAB_SETTING_REDIS_HOST="+viper.GetString("redis.address"))
	cmd.Env = append(cmd.Env, "CRAWLAB_SETTING_REDIS_PORT="+viper.GetString("redis.port"))
	if viper.GetString("redis.password") != "" {
		cmd.Env = append(cmd.Env, "CRAWLAB_SETTING_REDIS_PASSWORD="+viper.GetString("redis.password"))
	}
